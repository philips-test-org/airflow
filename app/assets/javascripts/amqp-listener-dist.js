...
var phoenix_2 = phoenix.Presence;
...
/* Callbacks for connecting to a channel.

   Callback functions:
     + new_msg: called when a new message is broadcast to the channel
     + update_presence: called when presence state changes
     + join_ok: called on a successful join
     + join_error: called when a join fails
     + on_close: called when a channel is closed gracefully
*/


/* Type of callbacks needed for:
     + publishing messages
     + binding to exchanges
     + unbinding from exchanges

   Callback functions:
     + ok: Called on successful [publish|bind|unbind]
     + error: Called on failed [publish|bind|unbind]
*/
...
var DEFAULT_BIND_CALLBACKS = {
  ok: function ok() {},
  error: function error(_reasons) {}
};

var DEFAULT_CONNECT_CALLBACKS = {
  newMsg: function newMsg(_msg) {},
  updatePresence: function updatePresence(_presence) {},
  joinOk: function joinOk(_response) {},
  joinError: function joinError(_response) {},
  ...
  this.connectToChannel = function (callbacks) {
    return new es6Promise(function (resolve, _reject) {
      callbacks = Object.assign({}, DEFAULT_CONNECT_CALLBACKS, callbacks);
      var connect = function connect(socket) {
        var user_id = socket.params.user_id;
        if (user_id) {
          _this.channel = connectToChannel(socket, "message:" + user_id, callbacks);
        }
        return _this.channel;
      };

      if (_this.socket) {
        resolve(connect(_this.socket));
      } else {
        var _socket = connectToSocket({ APM_URL: _this.APM_URL, onClose: callbacks.onClose });
        _this.socket = _socket;
        resolve(connect(_socket));
      }
    });
  };

  this.sendMessage = function (message, callbacks) {
    if (_this.channel) {
      sendMessage(_this.channel, message, callbacks);
    }
  };

  this.bindExchange = function (exchange, routingKey, callbacks) {
    if (_this.channel) {
      callbacks = Object.assign({}, DEFAULT_BIND_CALLBACKS, callbacks);
      bindExchange(_this.channel, exchange, routingKey, callbacks);
    }
  };

  this.unbindExchange = function (exchange, routingKey, callbacks) {
    if (_this.channel) {
      callbacks = Object.assign({}, DEFAULT_BIND_CALLBACKS, callbacks);
      unbindExchange(_this.channel, exchange, routingKey, callbacks);
    }
  };

  this.matchRoutingKey = matchRoutingKey;
}
...
// Connect to a channel over the socket.
var connectToChannel = function connectToChannel(socket, topic, callbacks) {
  var channel = socket.channel(topic, {});
  var presences = {};

  channel.on("new_msg", callbacks.newMsg);

  channel.on("presence_state", function (presenceState) {
    presences = phoenix_2.syncState(presences, presenceState);
    callbacks.updatePresence(presences);
  });

  channel.on("presence_diff", function (diff) {
    presences = phoenix_2.syncDiff(presences, diff);
    callbacks.updatePresence(presences);
  });
...
var matchRoutingKey = function matchRoutingKey(match, key) {
  if (match == "" && key == "") {
    return true;
  }
  ...
  if (match_token == "#") {
    return true;
  } else if (match_token == "*" && key_token != undefined) {
    return matchRoutingKey(match_rest.join("."), key_rest.join("."));
  } else if (match_token == key_token) {
    return matchRoutingKey(match_rest.join("."), key_rest.join("."));
  } else if (match_token != key_token) {
    return false;
  } else {
    throw "Unexpected condition in match routing key";
  }
};

var gen_id = function gen_id() {
  var id = Date.now().toString(36) + Math.random().toString(36).substr(2, 25);
  return id;
};
